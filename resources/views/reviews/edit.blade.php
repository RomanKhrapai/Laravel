...
@section('content')
    <h1 class="text-white">Form for updating reviews</h1>
    <span class="text-white">
        ID: {{ $review->id }}
    </span>
    <form class="" action="{{ route('reviews.update', ['review' => $review->id]) }}" method="post"
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" placeholder="Enter a title" id="title" class="form-control"
                value="{{ old('title', $review->title) }}" class="@error('title') is-invalid @enderror">
        </div>
        @error('title')
        ...
        <div class="form-group">
            ...
            <input type="text" name="description" placeholder="Enter a description" id="description" class="form-control"
                value="{{ old('description', $review->description) }}" class="@error('description') is-invalid @enderror">
        </div>
        @error('description')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        ...
        <div class="form-group">
            ...
            <input type="text" name="salary" placeholder="Enter a salary" id="salary" class="form-control"
                value="{{ old('salary', $review->salary) }}" class="@error('salary') is-invalid @enderror">
        </div>
        @error('salary')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        ...
        <div class="form-group">
            ...
            <input type="text" name="max_salary" placeholder="Enter a max_salary" id="max_salary" class="form-control"
                value="{{ old('max_salary', $review->max_salary) }}" class="@error('max_salary') is-invalid @enderror">
        </div>
        @error('max_salary')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
                    ...
                    <select class="js-example-basic-single" name="company_id">
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}"
                                {{ $company->id == old('company_id', $review->company_id) ? 'selected' : '' }}>
                    ...
                    <select class="js-example-basic-single" name="profession_id"
                        ...
                        @foreach ($professions as $profession)
                            <option value="{{ $profession->id }}"
                                {{ $profession->id == old('profession_id', $review->profession_id) ? 'selected' : '' }}>
        ...
        <div class="bg-white" id='skills'>
            @foreach ($skills as $skill)
                @if ($skill->profession_id == old('profession_id', $review->profession_id))
                    <div class="btn row">
                        <label class='form-check-label'> <input value="{{ $skill->id }}" type="checkbox"
                                class="form-check-input block" name="skills[]"
                                @if ((empty(old('skills')) && $review->skills->contains($skill->id)) || in_array($skill->id, old('skills', []))) @checked(true) @endif>
                            {{ $skill->name }}
                        </label>
                    </div>
                @endif
                    ...
                    <select class="js-example-basic-single" name="area_id">
                        @foreach ($areas as $area)
                            <option value="{{ $area->id }}"
                                {{ $area->id == old('area_id', $review->area_id) ? 'selected' : '' }}>
                                {{ $area->id }} - {{ $area->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="js-example-basic-single" name="nature_id">
                        @foreach ($natures as $nature)
                            <option value="{{ $nature->id }}"
                                {{ $nature->id == old('nature_id', $review->nature_id) ? 'selected' : '' }}>
                    ...
                    <select class="js-example-basic-single" name="type_id">
                        @foreach ($types as $type)
                            <option value="{{ $type->id }}"
                                {{ $type->id == old('type_id', $review->type_id) ? 'selected' : '' }}>
                                {{ $type->id }} - {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
